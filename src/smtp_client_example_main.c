...
#define SENDER_MAIL         "don.kunzig@gmail.com"
#define SENDER_PASSWORD     "64533456Aa?"
#define RECIPIENT_MAIL       "don.kunzig@gmail.com"
...
static int write_and_get_response(mbedtls_net_context *sock_fd, unsigned char *buf, size_t len)
{
    ...
    do {
        ...
        if (ret <= 0) {
            printf("mbedtls_net_recv failed with error %x", ret);
            goto exit;
        }
...
static int perform_tls_handshake(mbedtls_ssl_context *ssl)
{
    ...
    printf(TAG, "Performing the SSL/TLS handshake...");
    ...
    printf(TAG, "Verifying peer X.509 certificate...");
    ...
    if ((flags = mbedtls_ssl_get_verify_result(ssl)) != 0) {
        ...
        printf(TAG, "Certificate verified.");
    }

    printf(TAG, "Cipher suite is %s", mbedtls_ssl_get_ciphersuite(ssl));
    ret = 0; /* No error */
...
static void smtp_client_task()
{
    ...
    printf(TAG, "Seeding the random number generator");
    ...
    printf(TAG, "Loading the CA root certificate...");
    ...
    printf(TAG, "Setting hostname for TLS session...");
    ...
    printf(TAG, "Setting up the SSL/TLS structure...");
    ...
    printf("Connecting to MAIL_SERVER, MAIL_PORT\n");
